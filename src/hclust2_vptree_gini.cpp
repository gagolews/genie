...
HClustVpTreeGini::HClustVpTreeGini(Distance* dist, RObject control) :
      HClustNNbasedGini(dist, control),
      root(NULL)
//    visitAll(false)
{
...
HClustVpTreeGini::~HClustVpTreeGini() {
...
HClustVpTreeGiniNode* HClustVpTreeGini::buildFromPoints(size_t left,
   size_t right, std::vector<double>& distances)
{
   ...
   if (right - left <= opts.maxLeavesElems)
   {
      ...
      ++stats.leafCount;
   #endif
      HClustVpTreeGiniNode* leaf = new HClustVpTreeGiniNode(left, right);
...
//    HClustVpTreeGiniNode* node = new HClustVpTreeGiniNode(vpi, left, left+1, (*distance)(vpi, indices[median]));

   HClustVpTreeGiniNode* node = new HClustVpTreeGiniNode(vpi, left, left+1, distances[indices[median]]);
...
void HClustVpTreeGini::getNearestNeighborsFromMinRadiusRecursiveLeaf(
   HClustVpTreeGiniNode* node, size_t index,
...
void HClustVpTreeGini::getNearestNeighborsFromMinRadiusRecursiveNonLeaf(
   HClustVpTreeGiniNode* node, size_t index,
   size_t clusterIndex, double minR, std::priority_queue<double>& bestR, double& maxR, NNHeap& nnheap)
{
   STOPIFNOT(node->vpindex != SIZE_MAX);
...
//    else {
      if (dist < node->radius) {
         if (node->childL &&  dist + node->radius > minR) {
...
void HClustVpTreeGini::updateSameClusterFlag(HClustVpTreeGiniNode* node)
{
   if (prefetch || node->sameCluster ||
