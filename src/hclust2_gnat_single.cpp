...
void HClustGnatSingle::chooseNewSplitPoints(HClustGnatSingleNode* node,
   size_t degree, size_t left, size_t right)
{
   ...
   if (true || opts.vpSelectScheme == 1) {
      ...
      {
         for (size_t i=0;i<candidatesNumber;++i) // permute
         {
      ...
      size_t lastAddedSplitPoint = node->splitPoints[0] = tmpIndex;
      ...
      for(size_t i = 0; i<degree-1; ++i)
      {
         ...
         //_indices[left+i] = tmp;
         lastAddedSplitPoint = node->splitPoints[i+1] = maxIndex;
      }
      ...
      for(size_t i = 0; i < node->splitPoints.size(); ++i)
      {
         Rcout << _indices[left+node->splitPoints[i]]  <<" ";
      }
      Rcout << endl;
   #endif

      vector<bool> canSwap(node->splitPoints.size(), true);
      std::sort(node->splitPoints.begin(), node->splitPoints.end());
      int index = 0;
      for (size_t i = 0; i < node->splitPoints.size(); ++i)
      {
         if (node->splitPoints[i] < degree)
         {

            canSwap[node->splitPoints[i]] = false;
            size_t tmp = _indices[left+node->splitPoints[i]];
            RCOUT("zostawie" << tmp << " na indeksie "<< node->splitPoints[i], 15)
            node->splitPoints[i] = tmp;
         ...
         else
         {
            ...
            _indices[left+node->splitPoints[i]] = _indices[left+index];
      ...
      for (auto iter = splitPointsRanges_small.begin(); iter != splitPointsRanges_small.end();iter++)
      {
         ...
         if (index1 != SIZE_MAX && index2 != SIZE_MAX) //mamy taki wpis, ze jest to odleglosc miedzy rzeczywiscie wybranymi split pointami
         {
   ...
   else { // choose split points at random
      for (size_t i=0; i<degree; ++i) // permute
      {
         size_t tmpIndex = i + left + (size_t)(unif_rand()*(right-left-i));
         std::swap(_indices[tmpIndex], _indices[left+i]);
      }
...
HClustGnatSingleNode* HClustGnatSingle::createNonLeafNode(size_t degree,
   size_t optdegree, size_t left, size_t right)
{
   ...
   chooseNewSplitPoints(node, degree, left, right); // sets node->splitPoints

#if VERBOSE > 15
   for(size_t i=0;i<node->splitPoints.size();i++)
   {
      RCOUT("splitpoint[" << i << "]=" << node->splitPoints[i], 15)
   }
#endif
   //Rcout << "boundaries" << endl;
   //printIndices();

   vector<size_t> boundaries = groupPointsToSplitPoints(node, left, right);
#if VERBOSE > 15
   ...
   size_t childLeft = left+degree; // we don't store VPs in children nodes
   size_t childRight;
   ...
   for (size_t i=0; i<degree; ++i)
   {
      if (node->maxindex < node->splitPoints[i])
         node->maxindex = node->splitPoints[i];

      /// @TODO: chooseDegree(i) here, no vector<size_t> degrees
      ...
      {
         node->children[i] = buildFromPoints(degrees[i], degrees[i], childLeft, childRight);
         if (node->maxindex < node->children[i]->maxindex)
            node->maxindex = node->children[i]->maxindex;
       }
...
vector<size_t> HClustGnatSingle::groupPointsToSplitPoints(HClustGnatSingleNode *node, size_t left, size_t right)
{
   /* @TODO - VERY FUTURE WORK:
    * Add new private array of size n == #points
    * set this_private_array[i] = Index_closest_VP
    * retval[i] = liczba puntow w grupie
    *
    * sort(_indices.begin()+left, _indices.begin()+right, custom_this_private_array_dependent_comparer)
    *
    * retval[i] == indeks, na ktorym sie zaczyna i-ta grupa punktow, czyli left+cumsum(retval)
    */

   vector< vector<size_t> > groups(node->splitPoints.size());
   vector<size_t> boundaries(node->splitPoints.size());
   vector<double> distances(node->splitPoints.size());
   for(size_t i=left+node->splitPoints.size();i<right;++i)
   {
      size_t mySplitPointIndex = 0;
      double mySplitPointDistance = (*_distance)(node->splitPoints[0], _indices[i]);
      distances[0] = mySplitPointDistance;
      for(size_t j=1;j<node->splitPoints.size();++j)
      {
         double d = (*_distance)(node->splitPoints[j], _indices[i]);
         distances[j] = d;
      ...
      for(size_t j=0;j<node->splitPoints.size();++j)
      {
   ...
   for(size_t i = 0; i < node->splitPoints.size(); ++i)
   {
      ...
      {
         _indices[left+node->splitPoints.size()+cumsum+j] = groups[i][j];
      }
      cumsum += groups[i].size();
      //Rcout << "cumsum =" <<cumsum << endl;
      boundaries[i] = left+node->splitPoints.size()+cumsum;
...
vector<size_t> HClustGnatSingle::chooseDegrees(size_t degree, size_t optdegree, size_t left, size_t allPointsCount, const vector<size_t>& boundaries)
{
   ...
   for(size_t i=0;i<boundaries.size(); ++i) {
      ...
      if (elementsCount <= opts.maxLeavesElems)
         degrees[i] = elementsCount;
      else {
         degrees[i] = std::max((size_t)2, (size_t)log2(elementsCount));
      }
   }
   return degrees;
...
HClustGnatSingleNode* HClustGnatSingle::buildFromPoints(size_t degree,
   size_t optdegree, size_t left, size_t right)
{
#ifdef GENERATE_STATS
      ++stats.nodeCount;
   ...
   std::priority_queue<HeapNeighborItem>& heap )
{
   ...
   if (node->sameCluster) {
      if (node->degree == SIZE_MAX) {
         ...
         if (ds.find_set(node->splitPoints[0]) == clusterIndex) return;
      }
   }
   ...
   /* @TODO - pomysl
    * 1. liczymy distance od index do kazdego VP
    * 2. posortuj tablice T indeksow wzgledem tej odleglosci rosnaco
    * 3. dla kazdego i:
    *       jesli warto przejrzec  T[i]
    *            wejdz rekurencyjnie do T[i]
    *
    * EWENTUALNIE:
    *    znajdz najblizszy VP
    *    odwiedz go rekurencyjnie -> znajdziesz maxR oszacowanie (na pewno podczas prefetch)
    *    rozpatrz to samo co w oryginalnym paperze, ale z nowym maxR (wykluczajac juz odwiedzony)
    */
   ...
   size_t doTegoNieWchodzimy = SIZE_MAX;
   if(prefetch)
   {
      ...
      for(size_t i = 0; i < node->degree; i++)
      {
         if(node->children[i])
         {
            double d = distances[i] =  (*_distance)(node->splitPoints[i], index);
            if(d < minD)
            {
               minD = d;
               minIndex = i;
      ...
      //int a=5;
      getNearestNeighborsFromMinRadiusRecursive(node->children[minIndex], index, clusterIndex, minR, maxR, heap);
      doTegoNieWchodzimy = minIndex;
      //shouldVisit[minIndex] = false;
      //RCOUT("maxR = " << maxR, 1);
   }
   ...
   for(size_t i=0;i<node->degree; ++i) //4. z artykulu
   {
      if(shouldVisit[i])
      {
         ...
         //2. z artykulu
         double dist;
         if(prefetch && distances[i] != -1)
            dist = distances[i];
         else
         {
            dist = (*_distance)(pi, index);
            distances[i] = dist;
         }
   ...
   if (prefetch || node->sameCluster) return;
   ...
   for(size_t i = 1; i<node->degree; ++i)
   {
   ...
   for(size_t i=0; i<node->degree; ++i)
   {
      ...
      {
         size_t currentCluster = ds.find_set(node->children[i]->splitPoints[0]);
         if(currentCluster != commonCluster) return;
      }
   }
...
void HClustGnatSingle::FindNeighborTest(size_t index, double R)
{
   ...
   while( !heap.empty() ) {
      Rcout << heap.top().index+1 << ", " << heap.top().dist << endl;
      heap.pop();
   }
}
   ...
   for(size_t i = 0; i < n->children.size(); ++i)
   {
      ...
      if(n->children[i])
      {
         Rprintf("\"%llx\" -> \"%llx\" [label=\"child\"];\n",
            (unsigned long long)n, (unsigned long long)(n->children[i]));
         print(n->children[i]);
      }
   }

   if (n->degree == SIZE_MAX) {
      for (size_t i=n->left; i<n->right; ++i)
         Rprintf("\"%llx\" -> \"%llu\" [arrowhead = diamond];\n", (unsigned long long)n, (unsigned long long)_indices[i]+1);
   }
   else {
      Rprintf("\"%llx\" [label=\"(", (unsigned long long)n );
      for (size_t i = 0; i < n->degree; ++i)
      {
         if(i < n->degree - 1)
            Rprintf("%llu, ", (unsigned long long)n->splitPoints[i]+1);
         else
            Rprintf("%llu", (unsigned long long)n->splitPoints[i]+1);
...
void HClustGnatSingle::print() {
   Rprintf("digraph gnat {\n");
   Rprintf("size=\"6,6\";\n");
...
void hclust_gnat_single_test(RObject distance, RObject objects, RObject control=R_NilValue, int index=0, double R=0) {
   ...
   hclust.FindNeighborTest(index-1, R);
   if (dist) delete dist;
}
